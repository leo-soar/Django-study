...
- 模型类
	...
	class Note(models.Model):
	    ...
	    #时间字段 - 数据库里存储的UTC时间,模板层中会根据settings.py中的TimeZone配置修正现实时间
...
**如果没有Expire 和cache-control  火狐会取当前时间 减去 last-Modifide的值 /10 来作为过期时间**
...
**此过程在 项目开始时确定使用django自带auth   并在 makemigration   migrate之前添加好** 
...
- nginx 配置 
	...
	- 修改nginx 的配置文件 vim /etc/nginx/sites-enabled/default
...
- Nginx配置中添加新配置
	...
	```nginx
	![20200404014011](E:\PythonStudy\第三阶段后端\Django\images\20200404014011.jpg)# file : /etc/nginx/sites-enabled/default
